<template>
	<view>
		...
		<view class="list" v-for="(item,index) in likeList" :key="item.id">
			<view class="row">
		...
		<view class="tishi" v-if="likeList.length==0">
			没有数据亲！
		</view>
	</view>
</template>
...
<script>
	...
	export default{
		...
		async onLoad(e){
			...
			var result =await search(keywords);
			console.log(result);
			var likeList = result.data.list;
			if (likeList==null) {
				return;
			}else{
